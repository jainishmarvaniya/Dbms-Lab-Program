...
SELECT CNAME FROM DEPOSIT WHERE ADATE>1
...
SELECT TOP 5 LOANNO,CNAME FROM BORROW WHERE BNAME!='ANDERI'
...
SELECT * FROM BORROW WHERE (LOANNO BETWEEN 250 AND 600) AND (AMOUNT>1000 AND AMOUNT<=7000)
...
SELECT * FROM BORROW WHERE LOANNO>3000 AND LOANNO%3 !=0
...
SELECT * FROM DEPOSIT  WHERE AMOUNT>2000  AND  ACTNO BETWEEN 100 AND 110 AND ADATE>1-3-1995 OR ADATE<27-3-1996
--5. Retrieve all odd/even value loan number from Borrow table.
SELECT * FROM BORROW WHERE LOANNO%2!=0
SELECT * FROM BORROW WHERE LOANNO%2=0

-- comment mate short cut key ---> ctrl+kc
 --uncomment mate short cut key ---> ctrl+ku